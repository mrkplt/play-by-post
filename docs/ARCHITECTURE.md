...
- **`SceneSummary`** — one per scene (unique `scene_id`), holds the campaign-log prose; `generated_at` non-null marks AI provenance, `draft` gates visibility. Written by `SceneSummaryJob` (worker) after a scene resolves when the game has AI summaries on. **Async delivery to the scene page is a broadcast, not polling** (Fizzy #115/#116): the job finishes and `SceneSummaryBroadcast` (`app/models/scene_summary_broadcast.rb`) pushes a Turbo Stream over Action Cable / Solid Cable to the viewers waiting on the page. Because a summary's visibility differs per viewer (a draft → managers only; an AI summary → hidden from `hidden`-preference viewers — `SceneSummary#visible_to?`), the broadcast is **partitioned by visibility class** (`SceneSummaryVisibility`: `manager` / `plain` / `hidden`): the scene page subscribes each viewer to `[scene, :summary, <their class>]` and the worker broadcasts only to the classes entitled to see the summary. `SceneSummaryChannel` authorizes each subscription (a viewer may join only their own class's stream; the signed stream name plus this check is the gate). The `cable` message-backplane DB is on the shared `dbdata` volume — see `docs/CONFIGURATION.md` "Four SQLite databases". The asset itself carries only provenance (`generated_at`) — **accounting (who requested the generation, whose key paid, cost, model, token counts) lives in `AiGeneration`** (`app/models/ai_generation.rb`), a separate append-only audit trail: one row per BYOK-funded generation, written once by the worker inside the same transaction as the summary upsert, never updated or deleted, and carrying no associations (a plain `asset_type`/`asset_id` pair, not polymorphic) so no cascade — including `GamePurgeJob` or deleting a User — can ever reach or orphan it; the row outlives its asset. `Ai::UserGeneration` (`app/services/ai/user_generation.rb`) is the one engine every BYOK feature routes through to produce that accounting: it wraps `Ai::Funding`'s pool/failover and hands back a `Result` (body, model, tokens, cost, `funded_by`) so there is no path to an OpenRouter call that skips producing what the audit row needs. `SceneSummaryJob` is the current sole caller — it builds the prompt via `SceneSummaryPrompt`, calls `Ai::UserGeneration`, and writes the summary + `AiGeneration` row atomically.
- **`Post` — live scene chat** (Fizzy #123). A post created or edited by anyone appears/updates in place for **every** viewer on the scene, not just the submitter, over the same Turbo/Solid Cable backplane the summary broadcast uses. The scene page subscribes each viewer to one scene-wide `[scene, :posts]` stream (`PostsChannel` authorizes it via `ScenePolicy#show?` — game access + the private-scene gate — modeled on `SceneSummaryChannel`); `PostBroadcast` (`app/models/post_broadcast.rb`) appends a created post (and clears the empty state) or replaces an edited one by `dom_id`, from `PostsController#create`/`#update`. No visibility partitioning: a new post is unread for everyone but the author, OOC hiding is client-side (`ooc-filter` re-applies to streamed nodes via a MutationObserver), and the per-viewer **Edit** affordance is revealed client-side (`post-edit-affordance` controller, from the post's author id + edit window) so the single broadcast render stays viewer-neutral. The submitter's own `create` response appends their post immediately (reliable, no cable round-trip); their tab also receives the broadcast, a byte-identical append sharing the post's `dom_id`. Turbo's append-by-existing-id would *detach and re-append* the live node (clobbering the client-revealed Edit link mid-interaction — a real race, caught as a system-spec flake), so the `stream-dedupe` controller on `#posts` cancels an append whose children all already exist: whichever copy arrives second is a no-op by construction and never touches the DOM.
- **In-place mutations** (Fizzy #123). A user-initiated mutation that stays on the current screen answers with a Turbo Stream that swaps just the affected control/section and drops a toast into the always-present `#toast_layer`, instead of `redirect_to` (a full-page reload). The shared plumbing is `InPlaceRender` (`app/controllers/in_place_render.rb`: `toast_stream`, `game_controls_stream`, `turbo_or_redirect`, `flash_now`) plus, for long-form form editors, `InPlaceSave`/`SaveOutcome`. Every convertible form already submits via Turbo, so these are controller-side. Only **true navigations** still redirect: creating a new resource you then land on, deleting the resource you are viewing, a major state transition that re-renders most of the screen (scene resolve/join, character archive/restore), and the invitation-accept sign-in landing. Authorization denials always redirect.
