...
module Crypto
  # Decrypts the browser's hybrid-encrypted sealed value envelope back to the
  # plaintext value, using a PrivateKey. Keypair generation lives separately
  # in KeypairGenerator; the envelope shape is Blob.
  #
  # == Envelope / blob format (browser encrypts, this class decrypts)
  #
  # The browser holds the plaintext value only in memory and never sends it
  # unencrypted. It fetches PublicKey#public_key (PEM, RSA-2048) and, using
  ...
  #   3. AES-GCM-encrypt the UTF-8 plaintext value with that key + IV
  ...
  #
  # This class is the only place that ever holds the decrypted plaintext
  # value; callers are expected to use it and discard it, never persist it.
  class CryptoService
    extend T::Sig
    ...
    # exact format) back to the plaintext value. Raises DecryptionError on
    ...
    # errors are collapsed into one caller-facing error so a plaintext-value
    ...
    def decrypt(blob)
      ...
      raise DecryptionError, "failed to decrypt sealed value blob: #{error.message}"
    end
