...
class Shared::GamePagesListComponent < ApplicationComponent
  ...
  # Shared::PageRowActionsComponent still takes the raw game/page models (it
  # is outside this bundle's scope), so its two models are unwrapped here at
  # the boundary rather than threaded further through this component.
  sig { params(page: PagePresenter).returns(T.nilable(ViewComponent::Base)) }
  def row_controls(page)
    return nil unless can_manage?

    Shared::PageRowActionsComponent.new(game: T.cast(game.__getobj__, Game), page: T.cast(page.__getobj__, Page))
  end
end
