  ...
  describe "#attach" do
    ...
    it "attaches a slugified, dated filename and scope for a named game" do
      Timecop.freeze(Time.zone.local(2026, 3, 4)) do
        ...
        expect(AttachmentUploader).to have_received(:attach) do |args|
          ...
          expect(args[:kind]).to eq("export")
          expect(args[:user]).to eq(user)
          expect(args[:game]).to eq(game)
          expect(args[:original_filename]).to eq("the-lost-realm-export-2026-03-04.zip")
          expect(args[:export_scope]).to eq("The Lost Realm!")
    ...
    it "uses an all-games filename and scope when there is no game" do
      Timecop.freeze(Time.zone.local(2026, 3, 4)) do
        ...
        expect(AttachmentUploader).to have_received(:attach) do |args|
          expect(args[:game]).to be_nil
          expect(args[:original_filename]).to eq("all-games-export-2026-03-04.zip")
    ...
    it "strips characters that are not safe for a filename slug" do
      ...
      expect(AttachmentUploader).to have_received(:attach) do |args|
        expect(args[:original_filename]).to match(/\Afoo-bar-the-sequel-export-\d{4}-\d{2}-\d{2}\.zip\z/)
      end
    end
    ...
    it "collapses a literal double hyphen down to one" do
      ...
      expect(AttachmentUploader).to have_received(:attach) do |args|
        expect(args[:original_filename]).to match(/\Afoo-bar-export-\d{4}-\d{2}-\d{2}\.zip\z/)
      end
    end
    ...
    it "collapses every run of dashes, not just the first" do
      ...
      expect(AttachmentUploader).to have_received(:attach) do |args|
        expect(args[:original_filename]).to match(/\Afoo-bar-baz-export-\d{4}-\d{2}-\d{2}\.zip\z/)
      end
    end
