...
RSpec.describe Crypto::CryptoService do
  let(:keypair) { Crypto::KeypairGenerator.call }
  ...
  describe "#decrypt" do
    it "decrypts a well-formed envelope back to the original plaintext" do
      blob = Crypto::Blob.from_json(encrypt_like_a_browser(plaintext, keypair.public_key_pem))

      expect(service.decrypt(blob)).to eq(plaintext)
    end

    it "round-trips a real WebCrypto SubtleCrypto envelope (spec/fixtures/crypto_keypairs)" do
      fixtures_dir = Rails.root.join("spec/fixtures/crypto_keypairs")
      ...
      blob = Crypto::Blob.from_json(blob_json)
    ...
    it "raises DecryptionError with the underlying failure message when the ciphertext is tampered with (GCM auth failure)" do
      ...
      blob = Crypto::Blob.from_json(parsed.to_json)

      expect { service.decrypt(blob) }.to raise_error(Crypto::DecryptionError, /failed to decrypt sealed value blob/)
    end

    it "raises DecryptionError when the wrapped AES key was encrypted to a different keypair" do
      other_keypair = Crypto::KeypairGenerator.call
      blob = Crypto::Blob.from_json(encrypt_like_a_browser(plaintext, other_keypair.public_key_pem))
    ...
    it "raises DecryptionError, with a message naming the reason, when the ciphertext is too short to contain a GCM tag" do
      ...
      expect { service.decrypt(blob) }.to raise_error(
        Crypto::DecryptionError, /ciphertext too short to contain a GCM tag/
      )
    end
    ...
    it "raises DecryptionError when the ciphertext is exactly the tag length (0 bytes of actual ciphertext, still too short to be valid)" do
      blob = Crypto::Blob.new(
        wrapped_key: wrap_aes_key(OpenSSL::Random.random_bytes(32), keypair.public_key_pem),
        iv: Base64.strict_encode64(OpenSSL::Random.random_bytes(12)),
        ciphertext: Base64.strict_encode64("x" * (Crypto::CryptoService::AES_GCM_TAG_BYTES - 1))
      )
    ...
    it "does not raise the length-guard error once ciphertext reaches the tag-length boundary (guard is a strict <, not <=)" do
      ...
      expect(ciphertext_and_tag.bytesize).to eq(Crypto::CryptoService::AES_GCM_TAG_BYTES)
