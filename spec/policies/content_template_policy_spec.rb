...
RSpec.describe ContentTemplatePolicy do
  let(:user) { build_stubbed(:user) }
  let(:game) { instance_double(Game) }
  let(:template) { build_stubbed(:content_template) }

  subject(:policy) { described_class.new(user, template) }

  before { allow(template).to receive(:game).and_return(game) }

  describe "#manage? / #create? / #update? / #destroy? (GM only)" do
