...
# in ActionCable::TestHelper and needs the :test cable adapter). Request specs
# use the same helpers to confirm a controller action fired the broadcast it
# should. Feature specs wait for the browser's cable sockets with
# connect_turbo_cable_stream_sources (Turbo::SystemTestHelper) before triggering
# a broadcast, so the swap is observed rather than raced.
