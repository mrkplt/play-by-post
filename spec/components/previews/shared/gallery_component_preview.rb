...
class Shared::GalleryComponentPreview < ViewComponent::Preview
  ...
  def as_player
    ...
    render(Shared::GalleryComponent.new(game_files: game_files.to_a, game: game, can_manage: false))
  end

  def as_gm
    game       = Game.first || Game.new(id: 1, name: "Sample Game")
    game_files = GameFile.where(game: game).limit(12)
    render(Shared::GalleryComponent.new(game_files: game_files.to_a, game: game, can_manage: true))
  end
end
