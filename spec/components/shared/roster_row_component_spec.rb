...
RSpec.describe Shared::RosterRowComponent, type: :component do
  def rendered(**opts, &block)
    ...
    render_inline(described_class.new(**defaults.merge(opts)), &block)
    page
  end
  ...
  it "shows a crown when flagged" do
    expect(rendered(crown: true)).to have_css("svg, img", minimum: 1)
  end

  it "does not show a crown by default" do
    expect(described_class.new(name: "x", subtitle: "y").crown?).to be false
  end

  it "dims a removed/banned row" do
    expect(rendered(dimmed: true)).to have_css("div.opacity-70")
  end

  it "keeps a divider unless last" do
    expect(described_class.new(name: "x", subtitle: "y").row_classes).to include("border-b")
  end

  it "drops the divider on the last row" do
    expect(described_class.new(name: "x", subtitle: "y", last: true).row_classes).not_to include("border-b")
  end

  it "applies custom name and subtitle classes" do
    c = described_class.new(name: "x", subtitle: "y", name_class: "text-tint-blue-strong", subtitle_class: "text-tint-blue-soft")
