...
RSpec.describe GameFile, type: :model do
  describe "validations" do
    ...
    it "rejects files over 50MB" do
      game_file = build(:game_file)
      game_file.file.attach(io: StringIO.new("x" * (51 * 1024 * 1024)), filename: "big.pdf", content_type: "application/pdf")
      expect(game_file).not_to be_valid
    ...
    it "allows a file exactly 50MB" do
      game_file = build(:game_file)
      game_file.file.attach(io: StringIO.new("x" * 50.megabytes), filename: "exact.pdf", content_type: "application/pdf")
      expect(game_file).to be_valid
    end
