...
RSpec.describe "Mobile navigation", type: :feature do
  ...
  it "hides hamburger when menu is empty" do
    ...
    visit root_path
    expect(page).not_to have_css('[data-testid="hamburger"]', visible: true)
  end

  it "shows the hamburger icon on mobile" do
    visit root_path
    expect(page).to have_css('[data-testid="hamburger"]', visible: true)
  end

  it "hides nav menu by default on mobile" do
    visit root_path
    expect(page).to have_css('[data-testid="mobile-menu"][hidden]', visible: :all)
  end

  it "opens the nav menu when hamburger is tapped" do
    visit root_path
    find('[data-testid="hamburger"]').click
    expect(page).not_to have_css('[data-testid="mobile-menu"][hidden]', visible: :all)
    expect(find('[data-testid="mobile-menu"]')).to be_visible
  end

  it "clicking a nav link navigates away and collapses the menu" do
    visit root_path
    find('[data-testid="hamburger"]').click
    find('[data-testid="mobile-menu"]').click_link("Sign out")
    expect(page).not_to have_css('[data-testid="hamburger"]', visible: true)
  end
  ...
  it "nav links have a touch target of at least 44px" do
    ...
    height = page.evaluate_script(
      "document.querySelector('[data-testid=\"mobile-menu\"] a').getBoundingClientRect().height"
    )
