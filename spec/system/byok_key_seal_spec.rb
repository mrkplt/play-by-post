...
# resulting envelope is exactly what AiKeypairs::CryptoService can decrypt —
...
RSpec.describe "BYOK key sealing (AI Control Plane)", type: :feature do
  ...
  describe "with a keypair already generated" do
    ...
    let(:generated) { AiKeypairs::KeypairGenerator.call }

    before do
      create(:ai_keypair, owner: user, public_key: generated.public_key_pem, fingerprint: generated.fingerprint)
    end
    ...
    it "encrypts the pasted key client-side and stores only the sealed envelope" do
      ...
      plaintext = AiKeypairs::CryptoService.new(generated.private_key_pem).decrypt(blob)
    ...
    it "replacing an existing key overwrites the sealed envelope" do
      ...
      keypair = AiKeypair.find_by(owner: user)
      plaintext = AiKeypairs::CryptoService.new(generated.private_key_pem).decrypt(T.must(keypair.sealed_blob))
