...
- `bin/quality-metrics` tracks a `css_in_components_pct` metric: the percentage of CSS class tokens in ViewComponent templates (`app/components/**/*.html.erb`) versus all application view templates (`app/views/**/*.html.erb`, including mailer views — mailers can render components just as web views can)
