...
- `bin/quality-metrics` tracks a `css_in_components_pct` metric: the percentage of CSS class tokens (all frameworks) in ViewComponent templates (`app/components/**/*.html.erb`) versus plain application view templates (`app/views/**/*.html.erb`, mailer views excluded)
