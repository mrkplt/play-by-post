...
- **786 examples run without a database; 17 keep one.** The survivors are integration seams with no meaningful stub: ActionMailbox's routing harness, Active Storage blobs/variants, GlobalID job serialisation, per-membership scene selection, and `COALESCE(MAX(...))` ordering. Each is annotated in place with why.
