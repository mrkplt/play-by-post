...
**CI/CD topology — a green PR does not mean master will build:**
- The `build` job (Docker image + `assets:precompile` + push to GHCR) is gated to **push events on `master`/tags only** (`if: github.event_name == 'push' && github.ref == 'refs/heads/master'`). On a PR it shows **`SKIPPED`**. So a fully green PR proves the gates pass but **not** that the container builds — that is only validated **after merge**, when the master `build` job runs. Treat "all PR checks green" as necessary, not sufficient; for anything touching boot/initializers/assets/storage, the merge-to-master build is the real test. Do the local no-credentials precompile check (above) before merging.
